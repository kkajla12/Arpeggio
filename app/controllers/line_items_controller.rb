class LineItemsController < ApplicationController
  ...
  def create
    if current_cart.line_items.count == 0
      ...
      respond_to do |format|
        if @line_item.save
          format.html { redirect_to @line_item.cart, notice: "#{@line_item.product.name} has been added to cart." }
  ...
  def update
    ...
    respond_to do |format|
      if @line_item.update(line_item_params)
        format.html { redirect_to @line_item.cart, notice: 'Duration was successfully updated.' }
  ...
  private
    ...
    def line_item_params
      params.require(:line_item).permit(:product_id, :duration)
    end
end
